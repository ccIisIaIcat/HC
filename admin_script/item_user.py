...
def get_all_items(token):
    ...
    all_items = []
    page = 1
    page_size = 10  # 假设每页有10个物品

    while True:
        response = requests.get(
            f"{BASE_URL}/items?page={page}&page_size={page_size}",
...
def get_user_items(user_id, token):
    ...
    while True:
        response = requests.get(
            f"{BASE_URL}/user-items/{user_id}?page={page}&page_size={page_size}",
            headers=get_headers(token)
        )
    ...
    print(f"用户共有 {len(all_items)} 个物品")
    return all_items
...
for v in all_items:
    print(v["ID"],v["name"])
    result = add_item_to_user(3, v["ID"], 1, "饭团猫", user_token)
    print(result)
